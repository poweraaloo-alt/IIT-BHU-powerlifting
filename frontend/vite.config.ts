...
export default defineConfig(async () => {
  ...
  return {
    ...
    // not from the domain root, so every asset URL Vite emits needs this prefix.
    // Set GITHUB_PAGES=true in the build step (see the Actions workflow) and swap
    // "IIT-BHU-powerlifting" for your actual repo name.
    base: process.env.GITHUB_PAGES === "true" ? "/IIT-BHU-powerlifting/" : "/",
    plugins: [
      react(),
      tailwindcss(),
