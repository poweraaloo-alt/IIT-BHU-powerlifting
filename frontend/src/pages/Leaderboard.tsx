...
type SortField = keyof Pick<LeaderboardEntry, "rank" | "lifter" | "division" | "age" | "bodyweight" | "category" | "squat" | "bench" | "deadlift" | "total" | "dots">;

const sortOptions: { value: SortField; label: string }[] = [
  { value: "rank", label: "Rank" }, { value: "lifter", label: "Lifter" }, { value: "division", label: "Division" },
  ...
  return (
    <article className="relative border border-[#1E305B] bg-[#101B35] p-4" data-testid={`leaderboard-card-${entry.id}`}>
      ...
      <div className="flex items-start justify-between gap-4 pl-2">
        <div><p className="font-mono text-[10px] font-bold tracking-[0.18em] text-[#64748B]">#{entry.rank ?? "—"} / {entry.category}</p><h3 className="mt-2 font-heading text-xl font-bold uppercase tracking-tight">{entry.lifter}</h3><p className="mt-1 font-mono text-[10px] uppercase text-[#94A3B8]">{entry.division} / {value(entry.bodyweight)} kg / AGE {value(entry.age)}</p></div>
      ...
      <div className="relative mx-auto max-w-7xl px-5 pb-16 pt-10 lg:px-8 lg:pt-16">
        ...
        <section className="pt-8" data-testid="leaderboard-content">
          ...
          <div className="mt-6 grid gap-3 border border-[#1E305B] bg-[#101B35]/70 p-3 md:grid-cols-[minmax(220px,1fr)_auto_auto] md:items-center" data-testid="leaderboard-filters">
            <label className="relative block"><span className="sr-only">Search leaderboard</span><Search className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-[#64748B]" /><Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search lifter, division, or category..." className="h-11 rounded-none border-[#1E305B] bg-[#080F20] pl-10 text-sm text-white placeholder:text-[#64748B] focus-visible:border-[#E63946]" data-testid="leaderboard-search-input" /></label>
            <label className="flex items-center gap-2 whitespace-nowrap font-mono text-[10px] font-bold uppercase tracking-wider text-[#64748B]"><ArrowDownUp className="size-3" /><span>Sort by</span><select value={sortField} onChange={(event) => setSortField(event.target.value as SortField)} className="h-10 border border-[#1E305B] bg-[#080F20] px-2 text-[10px] text-[#E0E1DD] outline-none focus:border-[#E63946]" data-testid="leaderboard-sort-select">{sortOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label>
            <Button variant="outline" size="sm" className="h-10 rounded-none border-[#1E305B] bg-[#080F20] text-[#E0E1DD] hover:border-[#E63946] hover:bg-[#17264A]" onClick={() => setDescending((current) => !current)} data-testid="leaderboard-sort-direction">{descending ? <ArrowDown className="size-4" /> : <ArrowUp className="size-4" />} {descending ? "Descending" : "Ascending"}</Button>
          </div>
          ...
          {entries.length > 0 && <><div className="mt-5 hidden overflow-x-auto border border-[#1E305B] md:block" data-testid="leaderboard-table"><table className="w-full min-w-[1100px] border-collapse text-left"><thead className="bg-[#17264A]"><tr className="border-b border-[#1E305B]">{["Rank", "Lifter", "Division", "Age", "Bodyweight", "Category", "Squat", "Bench", "Deadlift", "Total", "DOTS"].map((header) => <th key={header} className="px-4 py-4 font-mono text-[10px] font-bold uppercase tracking-[0.16em] text-[#94A3B8]">{header}</th>)}</tr></thead><tbody>{entries.map((entry) => <tr key={entry.id} className="group border-b border-[#1E305B]/70 bg-[#101B35] hover:bg-[#17264A]" data-testid={`leaderboard-row-${entry.id}`}><td className="border-l-2 border-transparent px-4 py-4 font-mono text-sm text-[#64748B] group-hover:border-[#E63946]">{value(entry.rank)}</td><td className="px-4 py-4 font-heading text-base font-bold uppercase">{entry.lifter}</td><td className="px-4 py-4 text-sm text-[#E0E1DD]">{entry.division}</td><td className="px-4 py-4 font-mono text-sm">{value(entry.age)}</td><td className="px-4 py-4 font-mono text-sm">{value(entry.bodyweight)} kg</td><td className="px-4 py-4 font-mono text-xs text-[#FF6B73]">{entry.category}</td><td className="px-4 py-4 font-mono text-sm">{value(entry.squat)}</td><td className="px-4 py-4 font-mono text-sm">{value(entry.bench)}</td><td className="px-4 py-4 font-mono text-sm">{value(entry.deadlift)}</td><td className="px-4 py-4 font-mono text-sm font-bold">{value(entry.total)}</td><td className="px-4 py-4 font-mono text-sm font-bold text-[#FFB703]">{value(entry.dots)}</td></tr>)}</tbody></table></div><div className="mt-5 grid gap-3 md:hidden" data-testid="leaderboard-cards">{entries.map((entry) => <LeaderboardCard key={entry.id} entry={entry} />)}</div></>}
        </section>
      </div>
    </main>
  );
