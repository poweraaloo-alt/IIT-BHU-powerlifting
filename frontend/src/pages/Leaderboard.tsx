...
function LeaderboardCard({ entry, position }: { entry: LeaderboardEntry; position: number }) {
  return (
    <article
      className="relative border border-[#1E305B] bg-[#101B35] p-4"
      data-testid={`leaderboard-card-${entry.id}`}
    >
      <div className="absolute inset-y-0 left-0 w-1 bg-[#E63946]" />
      <div className="flex items-start justify-between gap-4 pl-2">
        <div>
          <p className="font-mono text-[10px] font-bold tracking-[0.18em] text-[#64748B]">
      ...
      <div className="mt-4 grid grid-cols-3 gap-3 border-t border-[#1E305B] pt-3">
        <div>
          <p className="metric-label">SQUAT</p>
          <p className="font-mono text-sm">{value(entry.squat)}</p>
        </div>
        <div>
          <p className="metric-label">BENCH</p>
          <p className="font-mono text-sm">{value(entry.bench)}</p>
        </div>
        <div>
          <p className="metric-label">DEADLIFT</p>
          <p className="font-mono text-sm">{value(entry.deadlift)}</p>
        </div>
        <div>
          <p className="metric-label text-[#E63946]">TOTAL</p>
          <p className="font-mono text-sm font-bold">{value(entry.total)}</p>
        </div>
        <div>
          <p className="metric-label text-[#E63946]">DOTS</p>
          <p className="font-mono text-sm font-bold">{value(entry.dots)}</p>
        </div>
      </div>
    </article>
  );
...
export default function Leaderboard() {
  ...
  const handleSearch = (term: string) => {
    ...
    if (term.trim().length >= 2) {
      trackEvent("leaderboard_search", { query_length: term.trim().length });
    }
  };
  ...
  const handleSortDirection = () => {
    ...
    trackEvent("leaderboard_sort_direction", {
  ...
  return (
    <main
      className="min-h-screen overflow-hidden bg-[#080F20] text-[#F8FAFC]"
    ...
    >
      ...
      <div className="relative mx-auto max-w-7xl px-5 pb-16 pt-10 lg:px-8 lg:pt-16">
        ...
        >
          <div>
            ...
            <h1 className="mt-4 font-heading text-5xl font-black uppercase leading-[0.92] tracking-[-0.04em] sm:text-6xl lg:text-8xl">
              IIT BHU
              <br />
              <span className="text-[#E63946]">Powerlifting</span>
              <br />
              Leaderboard
            </h1>
            <p className="mt-6 max-w-xl text-sm leading-7 text-[#94A3B8] sm:text-base">
              A clear view of every lifter, lift, total, and DOTS score. Sort the board by any field to find the numbers that matter.
            </p>
          </div>
        ...
        <section className="pt-8" data-testid="leaderboard-content">
          ...
          <div
            className="mt-6 grid gap-3 border border-[#1E305B] bg-[#101B35]/70 p-3 md:grid-cols-[minmax(220px,1fr)_auto_auto] md:items-center"
          ...
          >
            ...
            <label className="flex items-center gap-2 whitespace-nowrap font-mono text-[10px] font-bold uppercase tracking-wider text-[#64748B]">
              ...
              <select
                ...
                className="h-10 border border-[#1E305B] bg-[#080F20] px-2 text-[10px] text-[#E0E1DD] outline-none focus:border-[#E63946]"
            ...
            <Button
              variant="outline"
              size="sm"
              className="h-10 rounded-none border-[#1E305B] bg-[#080F20] text-[#E0E1DD] hover:border-[#E63946] hover:bg-[#17264A]"
          ...
          {entries.length > 0 && (
            <>
              ...
              <div className="mt-5 grid gap-3 md:hidden" data-testid="leaderboard-cards">
                {entries.map((entry, index) => (
                  <LeaderboardCard key={entry.id} entry={entry} position={index + 1} />
                ))}
              </div>
            </>
          )}
