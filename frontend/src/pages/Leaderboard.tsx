...
type SortField = keyof Pick<LeaderboardEntry, "lifter" | "team" | "division" | "age" | "bodyweight" | "category" | "squat" | "bench" | "deadlift" | "total" | "dots">;

const sortOptions: { value: SortField; label: string }[] = [
  { value: "lifter", label: "Lifter" }, { value: "team", label: "Team" }, { value: "division", label: "Division" },
  ...
  return (
    <article className="relative border border-[#1E305B] bg-[#101B35] p-4" data-testid={`leaderboard-card-${entry.id}`}>
      ...
      <div className="flex items-start justify-between gap-4 pl-2">
        <div><p className="font-mono text-[10px] font-bold tracking-[0.18em] text-[#64748B]">#{position} / {entry.category}</p><h3 className="mt-2 font-heading text-xl font-bold uppercase tracking-tight">{entry.lifter}</h3><p className="mt-1 font-mono text-[10px] uppercase text-[#94A3B8]">{textValue(entry.team)} / {entry.division} / {value(entry.bodyweight)} kg / AGE {value(entry.age)}</p></div>
...
export default function Leaderboard() {
  ...
  const entries = useMemo(() => {
    const filtered = (data?.entries ?? []).filter((entry) => `${entry.lifter} ${entry.team} ${entry.division} ${entry.category}`.toLowerCase().includes(search.trim().toLowerCase()));
      ...
      <div className="relative mx-auto max-w-7xl px-5 pb-16 pt-10 lg:px-8 lg:pt-16">
        ...
        <section className="pt-8" data-testid="leaderboard-content">
          ...
          <div className="mt-6 grid gap-3 border border-[#1E305B] bg-[#101B35]/70 p-3 md:grid-cols-[minmax(220px,1fr)_auto_auto] md:items-center" data-testid="leaderboard-filters">
            <label className="relative block"><span className="sr-only">Search leaderboard</span><Search className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-[#64748B]" /><Input value={search} onChange={(event) => handleSearch(event.target.value)} placeholder="Search lifter, team, division, or category..." className="h-11 rounded-none border-[#1E305B] bg-[#080F20] pl-10 text-sm text-white placeholder:text-[#64748B] focus-visible:border-[#E63946]" data-testid="leaderboard-search-input" /></label>
